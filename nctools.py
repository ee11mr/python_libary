...
def ncout3d_mod(file, lon, lat, p, val, name = None
            , longname = None, units = None):

# ; Function identical to ncout3d above except changes the order of 
# ; dimensions on the variable writtne to netcdf
# ; Will write as (lon,lat,plevs) without scrambling data
    ...
    vals=np.swapaxes(vals,0,2) # This is necessary to write netcdf files correctly, still unsure why
    ...
    if (type(longname) is str):
        title = longname
    write_var(ncdf_file, vals, name, 'f4', ('lon', 'lat', 'plev'), units, title)
